...
@section('content_header')
    <h2>
      Catedrático
      <a href="{{ route('catedratico.create') }}" class="btn btn-info">Nuevo</a>       
    </h2>
      ...
      <div class="card">
        <div class="card-header py-4">
          ...
          <div class="card-tools">
            <form action="{{ route('catedratico.index') }}" method="get" role="search">
              {{ csrf_field() }}
                ...
                <div class="card bg-light">
                  ...
                  <div class="card-footer">
                    <div class="text-right">
                      <form action="{{ route('catedratico.destroy', $value) }}" method="post">
                        <a class="btn btn-sm {{ $value->activo ? 'btn-info' : 'btn-success' }}" href="{{ route('catedratico.show', $value) }}" >{{ $value->activo ? 'DESACTIVAR' : 'ACTIVAR' }}</a>
                        <a class="btn btn-sm btn-warning" href="{{ route('catedratico.edit', $value) }}" ><i class="fas fa-pencil-alt"></i> Editar</a>
                        {{csrf_field()}}
                        <input name="_method" type="hidden" value="DELETE">
                        <button class="btn btn-sm btn-danger" type="submit"><i class="fas fa-trash-alt"></i> Eliminar</button>
                        <a class="btn btn-sm btn-dark" href="{{ route('catedraticoCurso.show', $value) }}" >Cursos asignados <span class="badge badge-danger">{{ count($value->cursos->where('activo', true)) }}</span></a>
                      </form>
                    </div>
                  </div>
                </div>
